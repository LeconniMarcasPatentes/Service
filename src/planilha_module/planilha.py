...
class Planilha:
    ...
    """
    def ler_formulario( filepath: str ) -> List['Planilha']:
        try:
            ...
            for index, row in df.iterrows( ):
                try:
                    ...
                    print( f"Erro ao processar a linha {index + 1}: chave {ke} não encontrada." )
                except Exception as e:
                    print( f"Erro inesperado ao processar a linha {index + 1}: {e}" )

            # ultimo_id = df.index[-1] + 1 if not df.empty else 1
